...
def play_capital_quiz(questions: int = 5, answers: int = 4, option: str = "country") -> None:
    ...
    for i, question_data in enumerate(quiz_questions):
        ...
        while True:
            try:
                ...
                print(f"Expected answer {Style.BRIGHT}[{possible_answers}]{Style.RESET_ALL}")
            else:
                break
        ...
        if selected_answer == correct_answer:
            ...
            print(f"\t{Fore.BLUE + Style.BRIGHT}Good answer, percentage: {percentage}%{Style.RESET_ALL}")
        else:
            print(f"\t{Fore.YELLOW + Style.BRIGHT}Bad answer, the correct answer is {correct_answer}, percentage: {percentage}%{Style.RESET_ALL}")
...
def main() -> None:
    ...
    parser.add_argument("-q", "--questions", type=int, default=5, help="Number of questions")
    parser.add_argument("-a", "--answers", type=int, default=4, help="Number of answer options")
    parser.add_argument("-o", "--option", type=str, default="country", help="Quiz option (e.g., country, us_state)")

    args = parser.parse_args()

    configure_logging(args.verbose)
    play_capital_quiz(questions=args.questions, answers=args.answers, option=args.option)
